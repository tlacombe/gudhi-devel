...
Moreover, the complexity of the data structures and of the algorithms used to  approximate shapes  rapidly grows as the dimensionality increases, which makes them intractable in high dimensions.  This curse of {\em dimensionality} is exemplyfied by the size of one of the simplest representations of a point set, namely its convex hull, whose complexity depends exponentially on the dimension. 
...
In information theory, signal and image processing, other pseudo-distances such as Kullback-Leibler, Itakura-Saito or Bregman divergences are prefered.  
...
{\em First}, the sofware platform will allow to experiment at a large scale, which is mandatory to 
...
{\em Third}, the platform will serve as a unique tool to communicate with the computational geometry community and with researchers from other fields. In return, we will get feedback from practionners that will help shaping the platform.
...
It will also provide an open platform with no equivalent in USA or Asia. We forsee the platform to become a catalyst for research in high-dimensional geometry inside and outside of the project.  
...
We will keep close contacts with research groups working in those domains and leap on opportunities arising from our new results and tools.
 In return, we will get feedback from practitionners which will help shaping the theoretical models and the software platform. Based on our experience with CGAL, we will undertake a vigorous action towards code diffusion in applied domains.
