...
void write_points( std::string file_name, Point_Vector & points)
{
  std::ofstream ofs (file_name, std::ofstream::out);
  for (auto w : points)
    {
    ...
    for (auto v: witness_complex.complex_vertex_range())
      {
        ...
        if (u < v && witness_complex.find(edge) != witness_complex.null_simplex()&&u<landmarks.size()&&v<landmarks.size())   
          {
