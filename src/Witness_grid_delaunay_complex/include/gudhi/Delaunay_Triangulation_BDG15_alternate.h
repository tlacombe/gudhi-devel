...
public:
    //constructor
    Thick_relaxed_witness_complex(Point_Vector L,int dim,Simplicial_complex &sc,double lambda,double eps,bool check,bool fullleaf_deltacheck,std::vector<bool> &boundary)
    {
        double spar;
        ...
        for(int i=0; i<L.size(); i++)//i represents point p...looping over all points as in algorithm 3 in the paper
        {
            ...
            for(int sim=0; sim<C.size(); sim++) //checks if the simplices of C are to be included are not
            {
                ...
                if(fullleaf_deltacheck==1)
                {
                    Point_Vector children,parents;
                    double len=4*lambda+8*eps;
                    int cnt=0;
                    bool should_insert=1;
                    ...
                    Point cente(dim,cent.begin(),cent.end());
                    parents.push_back(cente);
                    bool bndry=0;
                    ...
                    for(int a=0; a<parents.size(); a++)
                    {
                        ...
                        else
                        {
                            ...
                            if(a==parents.size()-1)
                            {
                                bool quit=0;
                                bool flop=0;
                                if(full_leaves.size()<=1||bndry==1)
                                {
                                    quit=0;
                                    if(full_leaves.size()==0)
                                    {
                                        should_insert=0;
                                        break;
                                    }
...
public:
    ...
    Delaunay_Triangulation_BDG15(Simplex_tree<> &delt,Point_Vector & L, int const dim,double lambda, const double eps,double const rho)
    {
        ...
        do
        {
            ...
            Thick_relaxed_witness_complex<Simplicial_complex> T(L1,dim,dt,lambda,eps,true,true,boundary);
