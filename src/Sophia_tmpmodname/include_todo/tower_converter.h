...
template<class ComplexStructure>
Tower_converter::Tower_converter(std::string outputFileName) : streamingType_(VERTICES)
{
...
void Tower_converter::add_insertion(std::vector<double> *simplex, double timestamp)
{
    ...
    if (complex_->insert_simplex(&transSimplex)) stream_simplex(&transSimplex, timestamp);
}
