 ...
 */
                                        ...
                                        , std::vector< Matrix_chain * > & chains_in_H )
    {
        ...
        Matrix_chain *chain_fp = *(chains_in_F.begin()); //col_fp, with largest death <d index.
        ...
        std::cout << chains_in_F.size() << " size\n";
        for(auto chain_f : chains_in_F) { std::cout << chain_f->birth() << " birth\n"; }
        std::cout << available_birth.size() << " ab size\n";

        /*std::cout << "---" ;
        std::cout << birth_ordering_.reverse_birth_order(chains_in_F.at(0)->birth(),chains_in_F.at(1)->birth()) << "  " <<
                     birth_ordering_.reverse_birth_order(chains_in_F.at(1)->birth(),chains_in_F.at(0)->birth()) << "---\n";*/
        ...
        /*(auto chain_f_it  = chains_in_F.rbegin(); //by increasing death
            (*chain_f_it)->paired_col() != chain_fp->paired_col();
            ++chain_f_it ) //chain_fp = begin() has max death*/
        { //find which reduced col has this birth
            std::cout << (*chain_f_it)->birth() << " birth\n";
