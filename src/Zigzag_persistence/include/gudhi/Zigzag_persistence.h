...
// Encoding Matrix types:
  ...
  , birth_(-3), birth_fil_(0), lowest_idx_(-1) {}
  ...
  , birth_(b), birth_fil_(fil), lowest_idx_(l) {}
  ...
  { birth_ = other->birth_; birth_fil_ = other->birth_fil_;}
// private:
  Column             * column_      ;
  Row                * row_         ;
  ...
  Simplex_key          lowest_idx_  ; //to update...
...
//Comparison function to sort intervals by length in the output persistence diagram
  struct cmp_intervals_by_log_length {
    ...
    { return abs(log2((double)p.b_) - log2((double)p.d_)) >  
             abs(log2((double)q.b_) - log2((double)q.d_)); }
  };
  struct cmp_intervals_by_length {
    cmp_intervals_by_length(){}
    bool operator()( interval_t p, interval_t q) 
    { return abs((double)p.b_ - (double)p.d_) >  
             abs((double)q.b_ - (double)q.d_); }
  };
...
{
  auto it1 = c1->col_.begin();   auto it2 = c2->col_.begin();
  while(it1 != c1->col_.end() && it2 != c2->col_.end())
  {
    if(it1->key_ < it2->key_) { ++it1; }
      ...
      if(it1->key_ > it2->key_) {
        ...
        c1->col_.insert(it1, *new_cell); //col link, in order
        ...
        c1->col_.erase(tmp_it); //remove from col
        delete tmp_ptr;
      }
    }
  }
  while(it2 != c2->col_.end()) { //if it1 reached the end of its column, but not it2
    Cell * new_cell = new Cell(it2->key_,self1);
    lowidx_to_matidx_[it2->key_]->row_->push_back(*new_cell); //row links
    c1->col_.push_back(*new_cell); 
    ++it2; 
  }
}
...
void compute_zigzag_persistence()
{
  ...
  { 
    ...
    if(cpx_->simplicial_map_flag(zzsh)) { 
      ...
      if(curr_fil_ != fil) { prev_fil_ = curr_fil_; curr_fil_ = fil; }    
                                          forward_arrow(zzsh); } 
    else                                { backward_arrow(zzsh);}
    ++num_arrow_;
  }
...
//Reduce the boundary of zzsh in the basis of cycles.
  ...
  { 
    ...
    new_col->col_.push_back( *new_cell ); //zzsh must have largest idx of the column
  ...
  while( paired_idx != NULL ) {
    ...
    for(auto &cell : col_low->column_->col_) { //Recuce with the column col_g 
  ...
  while(true) 
  {
    ...
    for(auto &cell : col_low->column_->col_) {
      res_insert = col_bsh.insert(cell.key_);
      if( !res_insert.second ) { col_bsh.erase(res_insert.first); } //1+1 = 0
      //o.w. insertion has succeeded.
    }
    ...
    if(col_bsh.empty()) 
    { surjective_reflection_diamond(zzsh, chains_in_F, chains_in_H); return; }
    //Else, keep reducing.
    col_low     =  &(*lowidx_to_matidx_[*(col_bsh.rbegin())]); //curr low index col
...
//Compute the chain   zzsh + \sum col_h, for col_h \in chains_in_H
  ...
  for( matrix_chain * idx_h : chains_in_H ) { 
    for(auto &cell : idx_h->column_->col_ ) {
      res_insert = col_bsh.insert(cell.key_);
      if( !res_insert.second ) { col_bsh.erase(res_insert.first); }
    }
  }
  //Create a new row&column in the matrix
  Column * new_col = new typename matrix_chain::Column();
  Row  * new_row   = new typename matrix_chain::Row();
  matrix_.emplace_front( new_col
                       , new_row
                       , (matrix_chain *)0 //in F
                       , cpx_->key(zzsh)
  ...
  { //add all indices in col_new with canonical order enforced by set col_bsh
    ...
    new_col->col_.push_back( *new_cell ); //insertion in column 
    lowidx_to_matidx_[idx]->row_->push_back( *new_cell ); //insertion in row
  }
  //Add the bottom coefficient for zzsh
  Cell * new_cell = new Cell(cpx_->key(zzsh), &(*chain_it));
  new_col->col_.push_back( *new_cell ); //zzsh has largest idx of all
  new_row->push_back( *new_cell );
  //Update the map 'index idx -> chain with lowest index idx' in matrix_
  lowidx_to_matidx_[cpx_->key(zzsh)] = chain_it;
}
                            ...
                            , std::vector< matrix_chain * > & chains_in_H )
{
  ...
  auto  chain_fp  =  *(chains_in_F.begin()); //col_fp, with lowest birth index.
  ...
  std::map< Simplex_key, matrix_chain * 
          , decltype(cmp_birth) > birth_to_fidx(cmp_birth); //available birth
  //for f1 to f_{p} (fi increasing by <=d), insertion in birth_to_fidx sorts by >=b
  for(auto chain_f : chains_in_F) { birth_to_fidx[ chain_f->birth() ] = chain_f; } 

  // birth_to_fidx[ chain_fp->birth() ] = chain_fp; //contains p elements, the birth of fp must be available to others
  //test if line above is necessary
  if(birth_to_fidx.find(chain_fp->birth()) == birth_to_fidx.end()) {
    std::cout << "Miss chain_fp in birth_to_fidx when performing surjective_diamond \n";
  }

  Simplex_key max_birth_fil_ = birth_to_fidx.begin()->second->birth_fil_;//for diag
  birth_to_fidx.erase(birth_to_fidx.begin()); //remove max birth.
  ...
  { //find which reduced col has this birth
    auto birth_it = birth_to_fidx.find((*chain_f_it)->birth());  
    if(birth_it == birth_to_fidx.end()) { //birth not available anymore
      auto max_available_birth_col = birth_to_fidx.begin(); //max available birth
      //curr_col <- curr_col + max_birth_col
      ...
      birth_to_fidx.erase(max_available_birth_col); //remove birth from availability
    }
    else { birth_to_fidx.erase(birth_it); } //birth not available anymore
  }
  ...
  for(auto other_col : chains_in_H) { //Compute (\sum col_h) in a set  
      for(auto &cell : other_col->column_->col_) {
      res_insert = col_bsh.insert(cell.key_);
      if( !res_insert.second ) { col_bsh.erase(res_insert.first); } //1+1=0
    }
  }
  ...
  {  
    Cell * new_cell = new Cell(idx, &(*chain_it));
    new_col->col_.push_back( *new_cell );  //add in column
...
//Update persistence diagram with left interval [fil(b_max) ; fil(m))
  ...
  if(max_birth_fil_ != prev_fil_) {
    persistence_diagram_.emplace_back( cpx_->dimension(zzsh)-1
                                     ...
                                     , prev_fil_);//cpx_->filtration(zzsh)); 
  }
}
...
//Sort by left-to-right order in the matrix_ (no order maintained in rows)
  sort( modified_columns.begin(),modified_columns.end()
...
/* 
 * Permute s and t, whose insertions are adjacent. The bloc matrix gives:
 *                                 c_t                  c_t
 *                                  +                    +
 *      c_t c_s                    c_s c_s              c_s c_t 
 *   t   1   1                  s   1   0            s   1   1  
 *   s   0   1      --> either  t   0   1      or    t   0   1
 *
 */
      ...
      switch( other_col->birth() ) {
        ...
        case -1: {//in G x G, c_s+c_t (in G) has death max<d{h,h'}=max{h,h'}  
          ...
          if( curr_p_col->lowest_idx_ < other_p_col->lowest_idx_ ) {//h < h' 
            plus_equal_column( other_p_col, other_p_col->column_ //for death h'
                                          , curr_p_col->column_ );//c_h' <- c_h+c_h'
            plus_equal_column( other_col, other_col->column_ 
                                        , curr_col->column_ );//c_t <- c_s+c_t
            return curr_col;//continue with c_s, still paired with c_h
          }//endif
          else {//h' < h
            plus_equal_column( curr_p_col, curr_p_col->column_ //for death h
                                         , other_p_col->column_ );//c_h <- c_h+c_h' 
            plus_equal_column( curr_col, curr_col->column_//still paired with c_h
                                       , other_col->column_);//c_s <- c_s+c_t
            return other_col; //continue with c_t, of min death h'
          }//endelse
        }//end case GG
    ...
    default: {//c_s is in F 
      switch( other_col->birth() ) {
        ...
        case -1: { //in F x G: c_s+c_t is in F, of max birth
          plus_equal_column(curr_col, curr_col->column_
                                    , other_col->column_);//c_s <- c_s+c_t, in F
          return other_col;
        }//end case FG
