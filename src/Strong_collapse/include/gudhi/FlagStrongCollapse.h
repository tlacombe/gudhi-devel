...
public:
    ...
    */
	...
	FlagStrongCollapse(std::size_t expRows, std::size_t expMaxSimp)
	{
		init();
    ...
    */
	...
	FlagStrongCollapse(const size_t & num_vertices, const Filtered_sorted_edge_list  &edge_t)
	{
		init();
 ...
 //        		sparseRowAdjMatrix.insert(row_keep->second,columns) = 1;	
	...
	void insert_new_edges(const Input_vertex_range & vertex_range)
	{
		if(vertex_range.size() == 2) // checks if it is an edge or not. This should be a new egde
		{
    		...
    		if(rw_b == vertexToRow.end())	 // If rw_b is  a new vertex
    		{    			
    			...
    			vertexToRow.insert(std::make_pair(vertex_range.begin(), rows));
    			rowToVertex.insert(std::make_pair(rows, vertex_range.begin())); 
    			vertices.emplace(vertex_range.begin());
    			rows++;
    			cols++;
    		}

    		else if(rw_e == vertexToRow.end())	 // If rw_e is  a new vertex
    		{
       			sparseRowAdjMatrix.insert(rows,rows)  = 1;  // Initializing the diagonal element of the adjency matrix corresponding to rw_e.
    			vertDomnIndicator.push_back(false);
				rowInsertIndicator.push_back(true);
				rowIterator.push(rows);
    			vertexToRow.insert(std::make_pair(vertex_range.end(), rows));
    			rowToVertex.insert(std::make_pair(rows, vertex_range.end())); 
    			vertices.emplace(vertex_range.end());
    			rows++;
    			cols++;
    		}
