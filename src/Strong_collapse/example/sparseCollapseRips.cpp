...
int main(int argc, char * const argv[])
{
	...
	Simplex_tree 		*coll_simplex_tree;
	...
	while(threshold <= end_thresold)
	{
		...
		for(int i = 0; i < repetetions; i++)
		{
			...
			if(manifold == 's' || manifold == 'S'){
				...
				// point_generator.generate_points_sphere(*point_vector, number_of_points/3, dimension+1, radius-0.05);
				// point_generator.generate_points_sphere(*point_vector, number_of_points/3, dimension+1, radius+0.05);
				// point_generator.add_noise(*point_vector, number_of_points, dimension+1, radius, 0.02);
				// point_generator.generate_grid_2sphere(*point_vector, number_of_points/2, radius);
			...
			else
			{
			 ...
			 // for(auto  pnts : *point_vector )
    //     		point_generator.print_point(pnts, dimension);

			stree 		= new Fake_simplex_tree();
			coll_tree 	= new Fake_simplex_tree();

			simplexTreeToCnt 	= new Simplex_tree();
			coll_simplex_tree 	= new Simplex_tree();
			...
			collapseTime += mat->strong_collapse();
			std::cout << "Collapse done !" << std::endl;
			std::cout << "Time of Collapse And Simplex tree formation : ~" << collapseTime/(i+1) << " ms\n" << std::endl;

			coll_tree = new Fake_simplex_tree(mat->fake_simplex_tree_collapsed());
			coll_simplex_tree = new Simplex_tree(mat->simplex_tree_collapsed());
			...
			colNumSimp		+= coll_simplex_tree->num_simplices();
		...
		std::cout << "Rips complex is of dimension " << originalDim << " with " << originalNumMxSimp << "  maximal simplices and " << originalNumSimpFrmSmplxtr << "/" << originalMaxNumSimp  << " simplices and " << number_of_points << " vertices." << std::endl;
