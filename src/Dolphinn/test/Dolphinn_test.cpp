...
BOOST_AUTO_TEST_CASE(knn_query) {
  ...
  std::vector<std::vector<std::pair<int, double>>> result;
	...
	dolphi.k_nearest_neighbors_query(queries, Q, 10, N/100, result);
	double mean=0;
	int no_nn =0;
  ...
  result2.push_back(dummy);
	dolphi.k_nearest_neighbors_query(queries, 1, 1, N*2, result2);
	int min_idx = 0;
	double min_dist = 500;
...
BOOST_AUTO_TEST_CASE(radius_query) {
  ...
  std::vector<int> result(Q);
	...
	dolphi.radius_query(queries, Q, 0.8, N/100, result);
	for(int i=0; i<Q;++i){
		if(result[i]!=-1){
			double d=0;
			for(size_t j=0;j<D;++j){
