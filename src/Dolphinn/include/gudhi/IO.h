 ...
 */
template<typename T>
void readfvecs(std::vector<std::vector<T>>& v, int N, int D, const char* filename) {
 ...
 */
void print_2D_vector(const std::vector<std::vector<std::pair<int, float>>>& v, const int N, const int D)
{
  for( int i = 0; i < N; ++i)
  {
  	for(int j = 0; j < D; ++j)
