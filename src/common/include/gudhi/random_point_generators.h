...
// Let's call L a polyline in a 2D ambient space. Its first point is (0, 0), 
// and the length of each segment is 1. We generate "num_points" configurations of
// this polyline.
// For each output point P, one configuration is generated and "num_samples" 2D points
// are sampled on L. P is the concatenation of the coordinates of these points
// => the ambient dimension is 2*num_samples, but the intrinsic dimension is num_segments.
template <typename Kernel>
std::vector<typename Kernel::Point_d>
generate_points_on_polyline_configurations(
  std::size_t num_points, int num_segments, int num_samples, bool uniform = false)
{
  typedef typename Kernel::Point_d Point;
  typedef typename Kernel::FT FT;
  ...
  // Generate positions of samples
  std::vector<double> sample_positions;
  sample_positions.reserve(num_samples);
  for (int j = 0; j < num_samples; ++j)
  {
    if (uniform)
      sample_positions.push_back(static_cast<double>(j * num_segments) / num_samples);
    else
      sample_positions.push_back(rng.get_double(0., num_segments));
  }

  for (std::size_t i = 0; i < num_points; ++i)
  {
    // Generate polyline configuration
    std::vector<FT> angles(num_segments);
    for (FT & angle : angles)
      angle = rng.get_double(0., 2. * CGAL_PI);

    std::vector<FT> ambient_point;
    ambient_point.reserve(2 * num_samples);
    // Generate samples
    for (int j = 0; j < num_samples; ++j)
    {
      FT pos_on_L = sample_positions[j];
      int segment_index = static_cast<int>(std::floor(pos_on_L));
      FT x = 0., y = 0.;
      for (int seg = 0; seg < segment_index; ++seg)
      {
        x += std::cos(angles[seg]);
        y += std::sin(angles[seg]);
      }
      x += (pos_on_L - segment_index) * std::cos(angles[segment_index]);
      y += (pos_on_L - segment_index) * std::sin(angles[segment_index]);

      ambient_point.push_back(x);
      ambient_point.push_back(y);
    }

    // CJTODO TEMP
    if (num_points < 30)
      std::cerr << k.construct_point_d_object()(ambient_point.begin(), ambient_point.end()) << "\n";

    points.push_back(k.construct_point_d_object()(ambient_point.begin(), ambient_point.end()));
  }
