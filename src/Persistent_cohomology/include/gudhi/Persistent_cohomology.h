...
/** \defgroup persistent_cohomology Persistent Cohomology
 ...
 \li type Cell_handle to manipulate simplices,
 \li method <CODE>int dimension(Cell_handle)</CODE> returning
 the dimension of a simplex,
 \li type and method <CODE>Boundary_cell_range
 boundary_cell_range(Cell_handle)</CODE> that returns
 ...
 value type <CODE>Cell_handle</CODE>,
 \li type and method
 <CODE>Filtration_cell_range filtration_cell_range ()</CODE>
 that returns a range giving
 access to all the simplices of the complex read in the order
 assigned by the indexing scheme,
 \li type and method
 <CODE>Filtration_value filtration (Cell_handle)</CODE> that returns the value of
...
class Persistent_cohomology {
 public:
  ...
  typedef typename Complex_ds::Cell_key Cell_key;
  typedef typename Complex_ds::Cell_handle Cell_handle;
...
// Compressed Annotation Matrix types:
  // Column type
  typedef Persistent_cohomology_column<Cell_key, Arith_element> Column;  // contains 1 set_hook
  // Cell type
...
// Sparse column type for the annotation of the boundary of an element.
  typedef std::vector<std::pair<Cell_key, Arith_element> > A_ds_type;
// Persistent interval type. The Arith_element field is used for the multi-field framework.
  typedef std::tuple<Cell_handle, Cell_handle, Arith_element> Persistent_interval;
  ...
  explicit Persistent_cohomology(Complex_ds& cpx)
      : cpx_(&cpx),
        ...
        zero_cocycles_(),                                // union-find -> Cell_key of creator for 0-homology
        ...
        cell_pool_() {
    Cell_key idx_fil = 0;
    for (auto sh : cpx_->filtration_cell_range()) {
 ...
 private:
  struct length_interval {
    ...
    bool operator()(Cell_handle sh1, Cell_handle sh2) {
 ...
 public:
  ...
  void compute_persistent_cohomology(Filtration_value min_interval_length = 0) {
    ...
    Cell_key key;
    for (auto v_sh : cpx_->skeleton_cell_range(0)) {  // for all 0-dimensional simplices
      key = cpx_->key(v_sh);

      if (ds_parent_[key] == key  // root of its tree
      && zero_cocycles_.find(key) == zero_cocycles_.end()) {
        persistent_pairs_.emplace_back(
            cpx_->cell(key), cpx_->null_cell(), coeff_field_.characteristic());
      }
    }
    for (auto zero_idx : zero_cocycles_) {
      persistent_pairs_.emplace_back(
          cpx_->cell(zero_idx.second), cpx_->null_cell(), coeff_field_.characteristic());
    }
// Compute infinite interval of dimension > 0
    for (auto cocycle : transverse_idx_) {
      persistent_pairs_.emplace_back(
          cpx_->cell(cocycle.first), cpx_->null_cell(), cocycle.second.characteristics_);
    }
  }
 ...
 private:
  ...
  void update_cohomology_groups_edge(Cell_handle sigma) {
    Cell_handle u, v;
    boost::tie(u, v) = cpx_->endpoints(sigma);

    Cell_key ku = dsets_.find_set(cpx_->key(u));
    Cell_key kv = dsets_.find_set(cpx_->key(v));
    ...
    if (ku != kv) {        // Destroy a connected component
      ...
      if (cpx_->filtration(cpx_->cell(idx_coc_u))
          < cpx_->filtration(cpx_->cell(idx_coc_v))) {  // Kill cocycle [idx_coc_v], which is younger.
        if (interval_length_policy(cpx_->cell(idx_coc_v), sigma)) {
          persistent_pairs_.emplace_back(
              cpx_->cell(idx_coc_v), sigma, coeff_field_.characteristic());
        }
        ...
        if (interval_length_policy(cpx_->cell(idx_coc_u), sigma)) {
          persistent_pairs_.emplace_back(
              cpx_->cell(idx_coc_u), sigma, coeff_field_.characteristic());
        }
        // Maintain the index of the 0-cocycle alive.
        if (ku != idx_coc_u) {
          zero_cocycles_.erase(map_it_u);
  ...
  void annotation_of_the_boundary(
      std::map<Cell_key, Arith_element> & map_a_ds, Cell_handle sigma,
      int dim_sigma) {
    ...
    for (auto sh : cpx_->boundary_cell_range(sigma)) {
      key = cpx_->key(sh);
      ...
      if (mult != coeff_field_.additive_identity()) {  // For all columns in the boundary,
        for (auto cell_ref : col->col_) {  // insert every cell in map_a_ds with multiplicity
          ...
          if (w_y != coeff_field_.additive_identity()) {  // if != 0
            result_insert_a_ds = map_a_ds.insert(std::pair<Cell_key, Arith_element>(cell_ref.key_, w_y));
   ...
   */
  void update_cohomology_groups(Cell_handle sigma, int dim_sigma) {
...
// Update the cohomology groups:
    if (map_a_ds.empty()) {  // sigma is a creator in all fields represented in coeff_field_
      ...
      for (auto map_a_ds_ref : map_a_ds) {
        a_ds.push_back(
            std::pair<Cell_key, Arith_element>(map_a_ds_ref.first,
                                                  map_a_ds_ref.second));
      }
   ...
   * where it worths 1.*/
  void create_cocycle(Cell_handle sigma, Arith_element x,
                      Arith_element charac) {
    Cell_key key = cpx_->key(sigma);
   ...
   * is "death_key".*/
  void destroy_cocycle(Cell_handle sigma, A_ds_type const& a_ds,
                       Cell_key death_key, Arith_element inv_x,
                       Arith_element charac) {
    // Create a finite persistent interval for which the interval exists
    if (interval_length_policy(cpx_->cell(death_key), sigma)) {
      persistent_pairs_.emplace_back(cpx_->cell(death_key)  // creator
          , sigma                                              // destructor
          , charac);                                           // fields
    }
      ...
      if (w != coeff_field_.additive_identity()) {
        ...
        if (curr_col->col_.empty()) {  // If the column is null
          ...
          if (result_insert_cam.second) {  // If it was not in the CAM before: insertion has succeeded
            ...
            Cell_key key_tmp = dsets_.find_set(curr_col->class_key_);
   ...
   */
  void plus_equal_column(Column & target, A_ds_type const& other  // value_type is pair<Cell_key,Arith_element>
                         , Arith_element w) {
 ...
 public:
  ...
  std::vector<Cell_key> ds_parent_;
  std::vector<Column *> ds_repr_;
  boost::disjoint_sets<int *, Cell_key *> dsets_;
  /* The compressed annotation matrix fields.*/
  Cam cam_;
  /*  Dictionary establishing the correspondance between the Cell_key of
  ...
  std::map<Cell_key, Cell_key> zero_cocycles_;
  /*  Key -> row. */
  std::map<Cell_key, cocycle> transverse_idx_;
