...
template<typename Point_container,
         ...
         typename FiltrationValue,
         typename Edge_t >
void points_to_edge_filtration(Point_container         &points,
                               Distance                      distance,
                               double                        nu,
                               double                        mu,
                               std::vector<FiltrationValue> &filtration_values,
                               std::vector<Edge_t>          &edge_filtration )
{
  ...
  size_t n = points.size();
  filtration_values.resize(n);
  // filtration_values[0] = std::numeric_limits<double>::infinity();//eps_0 = +infinity
  ...
  tbb::parallel_for(size_t(1), n, [&](size_t i) {//O(n^2)
    double dist = std::numeric_limits<double>::infinity();
    for(size_t j = 0; j < i; ++j) { //compute d(P_{i-1}, p_i)
      auto curr_dist = distance(points[i],points[j]); 
      if(dist > curr_dist) { dist = curr_dist; } //maintain shortest distance
    }// between ith point and the could made of points p1 ... p_i-1
    filtration_values[i-1] = dist; //d(P_{i-1}, p_i)
  });

  filtration_values[n-1] = 0;

  //turn filtration_value[i] into sparsity of {p_0, ... , p_i}
  for(size_t i = 1; i < n; ++i) {
    if(filtration_values[i] > filtration_values[i-1]) //make decreasing
    {  filtration_values[i] = filtration_values[i-1];  }//d_H(P_i, P)
  }

//to remove!!!
  // std::cout << "remove me\n";
  // for(int i=0; i < filtration_values.size()-1; ++i) {
  //   filtration_values[i] = filtration_values[i+1];
  // }  
  // filtration_values[filtration_values.size()-1] = 0;
  ...
  // for(size_t i = 0; i < n; ++i) //all vertices //truly parallelisable
...
//edges[i]==list of edges added and removed at eps_i
  ...
  edges_added.resize(n);  edges_removed.resize(n);//reserve also the edges_added[i]?
  //add vertices later
  for(size_t i = 0; i < n-1; ++i) {//all ascending arrows eps_i
  // tbb::parallel_for(size_t(0), n-1, [&](size_t i) {
    ...
    for(size_t j = 1; j <= i+1; ++j) {//nu eps_i+1 < length(p_j, p_k) <= mu eps_i
      ...
      while(it != dist_matrix[j].begin()) {
        ...
        edges_removed[i].emplace_back(it->first, j, filtration_values[i], false);
        ++number_of_arrows;
      }
    }
  } 
