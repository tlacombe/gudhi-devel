...
int main(int argc, char* const argv[]) {
  if (argc != 3) {
    std::cerr << "Usage: " << argv[0] << " path_to_file_graph max_dim \n";
    return 0;
  }
  ...
  std::cout << "Insert the 1-skeleton in the simplex tree in " << static_cast<double>(end - start) / CLOCKS_PER_SEC
            << " s. \n";
  ...
  for (auto f_simplex : st.filtration_simplex_range()) {
    ...
    std::cout << "- Star: \n";
    for (auto c_simplex : st.cofaces_simplex_range(f_simplex, 0)) {
      std::cout << "     "
                << "[" << st.filtration(c_simplex) << "] ";
