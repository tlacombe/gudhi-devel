...
public:
                                 ...
                                 , int                         dim_max )
    {
        ...
        cpx_                    = cpx;
        counter_insert          = 0;
        partial_zzfil_          = new std::vector< Simplex_handle >(); //TODO?
        ...
        //std::cout << partial_zzfil_->size() << " w0 \n";
        sh_it_ = partial_zzfil_->begin();
        //std::cout << cpx_->filtration(*sh_it_) << " t1 \n";
        ++edge_it_;
        for(auto & sh : *partial_zzfil_)
        { std::cout << counter_insert << " ---- counter\n";
...
private:
    ...
    Simplex_handle & dereference() const {
        /*std::cout << "start \n";
        std::cout << partial_zzfil_->size() << " s0 \n";
        std::cout << cpx_->filtration(*(partial_zzfil_->begin())) << " s1 \n";
        std::cout << cpx_->filtration(*sh_it_) << " s2 \n";*/
        return *sh_it_;
    }

    void increment()
    {
        std::cout << cpx_->key(*sh_it_) << "\n";
        ++sh_it_;
