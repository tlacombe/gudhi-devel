                   ...
                   std::string mesh_filename = "witness")
{
  ...
  write_witness_mesh(landmarks, landmarks_ind, simplex_tree, simplex_tree.complex_simplex_range(), is2d, l_is_v, mesh_filename+"_before_collapse.mesh");
  ...
  for (auto sh: simplices) {
    while (simplex_tree.dimension(sh) > simplices_by_dimension.size()+1)
      simplices_by_dimension.push_back(0);
    simplices_by_dimension[simplex_tree.dimension(sh)]++;
  }
  for (unsigned i = 0; i < simplices_by_dimension.size(); i++)
