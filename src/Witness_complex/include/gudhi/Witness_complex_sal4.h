...
#ifndef WITNESS_COMPLEX_SAL4_H_
#define WITNESS_COMPLEX_SAL4_H_
...
class Witness_complex_sal4 : public Witness_complex<Nearest_landmark_table_> {
private:
 ...
 *         of the pair range iterator needs to be 'std::pair<std::size_t, double>'.
  ...
  Witness_complex_sal4(Nearest_landmark_table_ const & nearest_landmark_table)
 ...
 private:
                     ...
                     SimplexWitnessMap* sw_map) const
  {
    ...
    for (auto aw_it = aw_list.begin(); aw_it != aw_list.end(); ++aw_it) {
      ...
      double norelax_dist2 = std::numeric_limits<double>::infinity();
      for (; l_it != end && l_it->second - alpha2 <= norelax_dist2; ++l_it) {
        Simplex_key sk = complex.find(Vertex_vector(1, l_it->first));
        // sk.insert(l_it->first);
      ...
      for (auto w: vw_pair.second) {
        ...
        for (; l_it != end && l_it->second - alpha2 <= norelax_dist2; ++l_it) {
          if (l_it->second > norelax_dist2)
            ...
            norelax_dist2 = l_it->second;
          auto sv_range = complex.simplex_vertex_range(vw_pair.first);
    ...
    for (auto vw_pair: *dim0_map) {
      for (auto w: vw_pair.second) {
        ...
        for (; l_it != end && l_it->second - alpha2 <= norelax_dist2; ++l_it) {
          ...
          (*dim1_map)[sh].emplace_back(WitnessForSimplex(l_it, w.witness_, norelax_dist2));
          w.witness_->increase();
          if (l_it->second < norelax_dist2)
            norelax_dist2 = l_it->second;
        }
      ...
      for (auto w: vw_pair.second) {
        ...
        for (; l_it != end && l_it->second - alpha2 <= norelax_dist2; ++l_it) {
          ...
          if (all_faces_in(vertices, &filtration_value, complex)) {
            complex.insert_simplex(vertices, filtration_value);            
            Simplex_handle sh = complex.find(vertices);
            (*curr_dim_map)[sh].emplace_back(WitnessForSimplex(l_it, w.witness_, norelax_dist2));
            w.witness_->increase();
          }          
          if (l_it->second < norelax_dist2)
            norelax_dist2 = l_it->second;
