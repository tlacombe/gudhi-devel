...
int main(int argc, char * const argv[]) {
  ...
  unsigned test_size = atoi(argv[1]);
  unsigned final_size = test_size, numeral = 1;
  std::cout << "Test   New     Old\n";
  while (final_size < test_size+1) {
    std::cout << final_size << ": ";
    results.clear();
    t.begin();
    Gudhi::subsampling::harpeled_mendel(k, points, final_size, 0, std::back_inserter(results));
    t.end();
    ...
    results2.clear();
    t.begin();
    Gudhi::subsampling::choose_n_farthest_points(k, points, final_size, 0, std::back_inserter(results2));
    t.end();
