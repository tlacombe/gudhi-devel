 ...
 * Hasse_diagram_cell. They implement functionality of a cell (generarator) 
 * of an arbitrary chain complex. They allow to access to both boundary 
 * and coboundary of cells elements in a constant time. In addition to that,
 * they store information about the cell like dimension, filtration. Additional 
 * information can also be stored when the template parameter Additional_information_ 
 * is set to a class to store the additional informations. 
 * 
 ...
 * 
 * Objects of a type Hasse_diagram_cell can be consructed by constitutive addition
 ...
 * cells is stored in two cinstitutive lines. 
 * In the first line, the cell id (which is supposed to be a positive integer from the range
 * between 0 and number of cells) that uniquelly identify the cell, its dimension are to be
 ...
 * vector of cell id and the incidence coeficient. 
 ...
 * for further details.
 * 
 * All the data structures to store filtered complexes currently avialable in 
