...
come with no or very limited guarantees. For example, Isomap~\cite{tsl-isomap-2000}  provides
a correct embedding only if the manifold is isometric to a convex open set of
$\R ^k$, where $k$ is the estimated dimension of the manifold,  and LLE~\cite{rs-lle-2000} can only reconstruct topological balls. {\em Topological methods} are intended to 
better approximate the sampled manifold,  by constructing  piecewise linear (PL)
...
\paragraph{Simplicial complexes.}  The simplest and most widely used PL approximations
are  the {\em simplicial complexes}~\cite{hh-ct-2010}. Simplicial complexes can be defined in any dimension and can be used as piecewise linear approximations of general shapes.
