...
#ifndef SRC_SIMPLEX_TREE_INCLUDE_GUDHI_SIMPLEX_TREE_SIMPLEX_TREE_ZIGZAG_CLASSES_H_
#define SRC_SIMPLEX_TREE_INCLUDE_GUDHI_SIMPLEX_TREE_SIMPLEX_TREE_ZIGZAG_CLASSES_H_
...
class Zigzag_edge {
 public:
  ...
  typename FilteredComplex::Vertex_handle u() {
    return u_;
  }
...
, boost::forward_traversal_tag > {
 public:
  ...
  Flagzigzagfiltration_simplex_iterator(FlagZigzagFilteredComplex * cpx) {
    ...
    if (!arrow_direction_) {
      std::cout << "Remove an edge from an empty complex.\n";
    }
    ...
    for (auto & sh : partial_zzfil_) {
      sh->second.assign_key(counter_insert);
      ++counter_insert;
    } //TODO to keep?
  }
...
#endif  // SRC_SIMPLEX_TREE_INCLUDE_GUDHI_SIMPLEX_TREE_SIMPLEX_TREE_ZIGZAG_CLASSES_H_
